...
**Finding 2 (non-blocking, fixed anyway):** the `ci.yml` `go vet` step's
comment ("this plugin's entry point is behind a `//go:build wasip1` tag, so
host `go vet` silently skips it") is only true where that build tag
actually exists. Verified `src/main.go`'s build tags directly: **stripe**
and **tax-de** do carry `//go:build wasip1`; **qrpay, demo, button-nosale**
do not. The `GOOS=wasip1 GOARCH=wasm go vet ./...` command is still correct
and still what matches `scripts/build.sh`'s real target either way — only
the stated justification was wrong for this repo. Fixed to the accurate,
